...
`oracle_signal` turns those two gaps into a trade call using a minimum reward:risk
ratio (default 1:2 - the winning side must be at least double the losing side):
upside_gap 50 vs downside_gap 20 -> 50 >= 2 x 20 -> BUY. If it were upside_gap 30 vs
downside_gap 20, that is less than double, so it is neither a clean BUY nor SELL -> HOLD.
"""
...
class OracleLabeler:
    """Adds forward-looking upper/lower bands and a reward:risk based trade signal.

    Usage:
        df = OracleLabeler(df, lookahead=20).label()
    """

    def __init__(self, df, lookahead=20, min_reward_risk_ratio=2.0):
        self.df = df.copy()
    ...
    def label(self):
        ...
        self.df = df
        known = df["oracle_signal"].notna().sum()
        print(
